...
public class BigramFrequencyPairs extends Configured implements Tool {
	...
	private static class MyReducer extends
		...
		private final static PairOfStrings MARGINAL = new PairOfStrings();
		...
		// Sum up all counts for this key
		int sum = 0;
		...
		if (key.getRightElement().equals("!")) {
			marginal = sum;
			MARGINAL.set(key.getLeftElement(), "");
			VALUE.set((float) marginal);
			context.write(MARGINAL, VALUE);
		} 
		else if (marginal > 0) {
			VALUE.set((float) sum / marginal); // Compute frequency
