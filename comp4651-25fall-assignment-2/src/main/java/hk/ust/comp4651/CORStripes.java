...
public class CORStripes extends Configured implements Tool {
		...
		protected void reduce(Text key, Iterable<MapWritable> values, Context context) throws IOException, InterruptedException {
			...
			for (MapWritable stripe : values) {
				for (Map.Entry<Writable, Writable> entry : stripe.entrySet()) {
					...
					IntWritable existing = (IntWritable) mergedStripe.get(word);
					int existingCount = (existing != null) ? existing.get() : 0;
					
					mergedStripe.put(new Text(word), new IntWritable(existingCount + count.get()));
				}
			}
	...
	public static class CORStripesReducer2 extends Reducer<Text, MapWritable, PairOfStrings, DoubleWritable> {
		...
		protected void reduce(Text key, Iterable<MapWritable> values, Context context) throws IOException, InterruptedException {
			...
			// Get Freq(A) for the current key word
