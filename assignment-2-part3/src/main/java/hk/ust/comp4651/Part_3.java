...
public class Part_3 {
    ...
    public static void delEmptyFilesRecursive(Path root) throws IOException {
        ...
        for (FileStatus status : statuses) {
            ...
            if (status.isDirectory()) {
                delEmptyFilesRecursive(path);
            } else if (status.isFile()) {
                if (status.getLen() == 0) {
                    ensureUnder(fs, root, path);        // Delete if file is empty
    ...
    public static void delBySuffixRecursive(Path root, String suffix) throws IOException {
        ...
        for (FileStatus status : statuses) {
            ...
            if (status.isDirectory()) {
                delBySuffixRecursive(path, suffix);
            } else if (status.isFile()) {
                // Delete if file name ends with the given suffix
                String fileName = path.getName();
                if (fileName.endsWith(suffix)) {
                    ensureUnder(fs, root, path);
                    fs.delete(path, false);
                }
            }
        }
